...
//useContext
// PropCard.jsx file ---- Has Props reusability [Prarent to Child to GrandChild - Passing Params to Parent to GrandChild ---> Props Drilling ] 

//useContext Helps Us Avoid Prop Drilling
// useContext Hook is used to Pass Parent to Sub-Child directly without using Child,[Avoids Props Drilling]
...
// Context() -- Parent component, in which maintain the state for user
// Main & Header are two child components, where as Header is the Sub-Child
// UserContext is Parent that Provides the Property 'user' to the children

const UserContext = React.createContext();
//Parent - Provider
function Context() {
  const [user] = React.useState({ name: "Seeli" });
  return (
...
// Without using the Props from Parent UserContext
// const Main = () => {
//   return (
//     <>
//       <Header />
//       <br />
//       <div>Main app content...Welcome !!!!  </div>
//     </>
//   );
// };

//With using the Props from Parent UserContext
const Main = () => {
  ...
  return (
    <>
      <Header />
      <br />
      <div>Main app content...Welcome !!!! To {user.name} </div>
    </>
  );
};
...
// useMemo hook in react library - Performance optimization
...
// What useMemo does is it memoize/remember the return value of a function. 
...
function Memo() {
    ...
    // Without useMemo() -- Calls expensiveFunction() for all the properties loike count, name
    // When any one of the property is changed the method is called and it reloads all the properties -- LEADS to POOR PERFORMANCE
   // To avoid it useMemo Hook is used......
    // Without useMemo()
   // const result = expensiveFunction(count)  // Without useMemo()
...
// Calls expensiveFunction(), when the count value is changed in Text element    
 // useMemo() remembers the value of the count alone and re-render the entire component -- Speed up the execution - Provides better performance
 
 // useMemo() hook to memoize the property/value when in is changed
 const result = useMemo(()=>{
    return expensiveFunction(count)
  },[count])   // arg 1- function , arg 2- dependency(property to memoize for the next function call) 
