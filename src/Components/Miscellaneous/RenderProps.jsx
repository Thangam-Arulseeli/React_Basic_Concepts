...
// CommonJS
const Swal = require("sweetalert2");  // npm install sweetalert2
...
class AnotherWrapper extends Component {
  render() {
    return (
      <BaseComponent
        render={(name) => <p style={{ marginTop: 30 }}>Hello!!! Welcome to Mr./Mrs.  {name}!!!! </p>}
      />
    );
  }
}
...
//Custom hook for the above functionality [RenderProps-Class component]
function useName(initialValue = "Harry Potter") {
...
function CustomHookAnotherWrapper() {
  ...
  return <p style={{ marginTop: 30 }}>Hello Welcome to Mr./Mrs.  {name}!!!!!</p>;
}
...
//Mouse Component   // Getting Mouse Position for the image rendering
...
// Actual Workings - Combines UI in CircularImage and Functionality 
class Mouse1 extends Component {
  ...
  render() {
    return (
      <div style={{ height: "200vh" }} onClick={this.handleMouseClick}> <h2>Click Me</h2>
        {/*
           Using the render prop to dynamically determine what to render in this component.
          */}
        {this.props.render(this.state)}
  ...
  render() {
    return (
      <div>
        ...
        <br />
         <Mouse render={(mouse) => <CircularImage mouse={mouse} />} /> 
